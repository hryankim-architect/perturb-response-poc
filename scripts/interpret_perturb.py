...
def main() -> int:
    ...
    print(f"\n  Spearman(imp_obs, imp_true) = {rho:.4f}  "
          "(does learned reliance match ground-truth structure?)")
    ...
    (AUDIT / "feature_importance.md").write_text(
        ...
        f"- **Spearman(importance_observed, importance_true) = {rho:.4f}** — the "
        "ranking learned from the *noisy observed* delta vs the ranking implied by "
        "the *ground-truth* delta. High agreement ⇒ the model recovered the real "
